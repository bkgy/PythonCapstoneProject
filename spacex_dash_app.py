...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names="Launch Site", 
        title="Successfull launches per Launch Sites")
        return fig
    else:
        filtered_df = spacex_df[spacex_df["Launch Site"]==entered_site]
        filtered_df=filtered_df.groupby(["Launch Site","class"]).size().reset_index(name="count")
        fig=px.pie(filtered_df,values="count",names="class",title="Total Success Launches for site")
        return fig       
...
def get_scatter_plot(entered_site,payload):
    ...
    if entered_site=='ALL':
        ...
        fig=px.scatter(filtered_df2[filtered_df2['Launch Site']==entered_site],x='Payload Mass (kg)',y='class',color='Booster Version Category',title="Success count on Payload mass for site")
        return fig
